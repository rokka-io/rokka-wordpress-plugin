...
class PluginFunctionsTest extends Rokka_UnitTestCase {
	...
	public function test_get_attachment_url() {
		$image_name = '2000x1500.png';
		$attachment_id = $this->upload_attachment( $image_name );
		$this->add_rokka_hash( $attachment_id );
		$expected_attachment_url = $this->get_rokka_url( $attachment_id, $image_name, $this->get_stack_name_from_size( 'full' ) );
		$attachment_url = wp_get_attachment_url( $attachment_id );
		$this->assertEquals( $expected_attachment_url, $attachment_url );
		$this->remove_rokka_hash( $attachment_id );
	}
	...
	public function test_get_attachment_image_srcset() {
		if ( function_exists( 'wp_get_attachment_image_srcset' ) ) {
			...
			$attachment_image_srcset = wp_get_attachment_image_srcset( $attachment_id, 'large' );
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $large_filename, $this->get_stack_name_from_size( 'large' ) ), $attachment_image_srcset ) );
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $medium_filename, $this->get_stack_name_from_size( 'medium' ) ), $attachment_image_srcset ) );
			if ( array_key_exists( 'medium_large', $attachment_meta['sizes'] ) ) {
				$medium_large_filename = $attachment_meta['sizes']['medium_large']['file'];
				$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $medium_large_filename, $this->get_stack_name_from_size( 'medium_large' ) ), $attachment_image_srcset ) );
			}
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $larger_filename, $this->get_stack_name_from_size( 'larger' ) ), $attachment_image_srcset ) );
			// the size huge shouldn't appear in srcset since it's bigger than max_srcset_image_width defined in WordPress
			$this->assertEquals( 0, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $huge_filename, $this->get_stack_name_from_size( 'huge' ) ), $attachment_image_srcset ) );
			$this->remove_rokka_hash( $attachment_id );
		}
	}
	...
	public function test_get_attachment_image() {
		...
		if ( function_exists( 'wp_get_attachment_image_srcset' ) ) {
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $large_filename, $this->get_stack_name_from_size( 'large' ) ), $attachment_image ) );
			// the requested size appears in src attribute and in srcset attribute
			$this->assertEquals( 2, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $medium_filename, $this->get_stack_name_from_size( 'medium' ) ), $attachment_image ) );
			$medium_large_filename = $attachment_meta['sizes']['medium_large']['file'];
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $medium_large_filename, $this->get_stack_name_from_size( 'medium_large' ) ), $attachment_image ) );
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $larger_filename, $this->get_stack_name_from_size( 'larger' ) ), $attachment_image ) );

			$huge_filename = $attachment_meta['sizes']['huge']['file']; // huge size of the image only exists in newer versions of WordPress since the size is bigger than the image it wasn't added in old versions.
			// the size huge shouldn't appear in srcset since it's bigger than max_srcset_image_width defined in WordPress
			$this->assertEquals( 0, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $huge_filename, $this->get_stack_name_from_size( 'huge' ) ), $attachment_image ) );
		} else {
			$this->assertEquals( 1, preg_match_all( $this->ger_rokka_url_regex_pattern( $attachment_id, $medium_filename, $this->get_stack_name_from_size( 'medium' ) ), $attachment_image ) );
		}
		$this->remove_rokka_hash( $attachment_id );
	}
}
