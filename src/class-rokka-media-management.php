	...
	public function print_custom_media_columns_data( $column, $post_id ) {
		...
		if ( 'rokka' === $column ) {
			...
			if ( $this->rokka_helper->is_allowed_mime_type( $post_id ) ) {
				if ( $this->rokka_helper->is_on_rokka( $post_id ) ) {
					$output .= '<p><a href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?action=rokka_delete_image&image_id=' . $post_id ), 'rokka_delete_image_' . $post_id ) ) . '" class="button delete rokka-delete-image" title="' . esc_html__( 'Remove image from rokka', 'rokka-image-cdn' ) . '"><span class="dashicons dashicons-trash"></span></a></p>';
				} else {
					$output .= '<p><a href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?action=rokka_upload_image&image_id=' . $post_id ), 'rokka_upload_image_' . $post_id ) ) . '" class="button button-primary" title="' . esc_attr__( 'Upload image to rokka', 'rokka-image-cdn' ) . '"><span class="dashicons dashicons-upload"></span></a></p>';
				}
			} else {
